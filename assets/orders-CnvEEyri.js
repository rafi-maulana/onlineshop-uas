import{l as e,s as t}from"./api-BIgxvzZm.js";var n=JSON.parse(localStorage.getItem(`user`)),r=JSON.parse(localStorage.getItem(`cart`))||[];document.getElementById(`cartCount`).textContent=r.reduce((e,t)=>e+t.quantity,0),e().then(e=>{e&&e.shop_name&&(document.getElementById(`logoText`).textContent=e.shop_name,document.title=`Pesanan Saya - ${e.shop_name}`)}).catch(()=>{});var i=new Intl.NumberFormat(`id-ID`,{style:`currency`,currency:`IDR`,maximumFractionDigits:0}),a={pending:{label:`Menunggu`,class:`bg-amber-500/10 text-amber-400 border-amber-500/20`,dot:`bg-amber-400`},processing:{label:`Diproses`,class:`bg-blue-500/10 text-blue-400 border-blue-500/20`,dot:`bg-blue-400`},shipped:{label:`Dikirim`,class:`bg-indigo-500/10 text-indigo-400 border-indigo-500/20`,dot:`bg-indigo-400`},completed:{label:`Selesai`,class:`bg-emerald-500/10 text-emerald-400 border-emerald-500/20`,dot:`bg-emerald-400`},cancelled:{label:`Dibatalkan`,class:`bg-red-500/10 text-red-400 border-red-500/20`,dot:`bg-red-400`}};function o(e){let t=[`pending`,`processing`,`shipped`,`completed`];return e===`cancelled`?{steps:[`pending`,`cancelled`],current:`cancelled`,isCancelled:!0}:{steps:t,current:e,currentIdx:t.indexOf(e),isCancelled:!1}}function s(e){let{steps:t,currentIdx:n,isCancelled:r}=o(e);return r?`
